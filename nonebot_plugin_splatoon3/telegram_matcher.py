...
async def _(matcher: Matcher, event: EventTg):
...
async def _(matcher: Matcher, event: EventTg):
    ...
    if re.search("^随机武器.*$", plain_text):
        ...
        if not get_weapon_info_test():
            ...
            # 发送消息
            await matcher.finish(SegmentTg.image(file=img, cache=False))
        ...
        if img is None:
            msg = "近期没有任何祭典"
            msgm = SegmentTg.text(msg)
        else:
            # 发送图片
            msgm = SegmentTg.image(file=img, cache=False)
...
async def _(matcher: Matcher, event: EventTg):
    ...
    if re.search("^清空图片缓存$", plain_text):
        ...
        msg = "武器数据更新完成"
        await matcher.send(SegmentTg.text(msg_start))
        try:
